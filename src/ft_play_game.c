...
/*   Updated: 2025/05/17 05:53:16 by flren            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_map_render(t_game *game)
{
	...
	while (game->map[r])
	{
		...
		{
			if (game->map[r][c] == '1')
				ft_put_img(game, r * 64, c * 64, game->img_wall);
			else if (game->map[r][c] == 'P')
				ft_put_img(game, r * 64, c * 64, game->img_player);
			else if (game->map[r][c] == '0')
				ft_put_img(game, r * 64, c * 64, game->img_floor);
			else if (game->map[r][c] == 'E')
				ft_put_img(game, r * 64, c * 64, game->img_exit);
			else if (game->map[r][c] == 'C')
				ft_put_img(game, r * 64, c * 64, game->img_coins);
			c++;
		}
		r++;
	}
